...
<tr>
	...
	<?php  
		for ($j = 0 ; $j < sizeof($aCon) ; $j++) {
			if ($aCon[$j]["id"] != $_POST["e"]) {
	?>
		<option value="<?= $aCon[$j]["id"] ?>"><?= utf8_encode($aCon[$j]["nom"]); ?></option>
	<?php		} else { ?>
		<option value="<?= $aCon[$j]["id"] ?>" selected><?= utf8_encode($aCon[$j]["nom"]); ?></option>
	<?php 		}
		} ?>
	</select>&nbsp;<input type="text" value="Jugador" name="j" size="10"></div>
	<center><font color="#000000">(No és necessari sel·leccionar un equip)</font></center>
	</td>
</tr>
...
if (sizeof($consulta1) > 1)   { ?>		 
	...
	<table border="0" class="moduletable" align="center" width="100%">
		...
		<?php	for ($i = 0; $i < sizeof($consulta1); $i++)  { ?>		
					...
					<?=$consulta1[$i][1]?> (<?=$consulta1[$i][2]?>)</a></tr></div></th>
			<?php } ?> 
			<br>
...
<?php 
	...
	{ // si hem trobat 1 fila  	
		...
		<th colspan="7"><div align="center">Estadístiques de <?= $nomJug ?></div></th>
		<tr>		
		...
		* En <font color="#969696"><b>gris </b></font>apareixen els concursants que no han alineat el jugador per aquesta jornada.<br>
	<?php 
	} 
else { //sizeof = 0 ?>
	<tr><td>&nbsp;</td></tr>
...
?>
<tr><td>&nbsp;</td></tr>
<tr><td colspan="4"><div align="center"><input type="submit" value="Buscar"></div>
...
<?php if ($idJug > 0) {
	...
	<th colspan="4"><div align="left">Punts per Jornada</div></th>
<?php
...
?>
	<tr>
		<td width="20%"><div align="left">J<?= $j+1 ?></div></td>
		<td width="20%"><div align="left"><?php if ($p < 0) { ?><font color="#FF0000"><?php } else{?><font color="#04B404"><?php } ?><?= $p ?></div></td>
		<td width="60%"><div align="left"><?php if ($p < 0) { ?><font color="#FF0000"><?php } else{?><font color="#04B404"><?php } ?><?= $X ?></div></td>
